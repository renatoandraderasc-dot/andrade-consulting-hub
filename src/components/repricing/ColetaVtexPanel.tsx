...
import { Play, Loader2, Plus, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
...
const ColetaVtexPanel = () => {
  ...
  const carregar = useCallback(async () => {
    const { data } = await supabase
      ...
      .eq("ativo", true)
      .order("nome");
    setConcorrentes((data as Concorrente[]) || []);
  }, []);

  useEffect(() => { carregar(); }, [carregar]);

  // recupera job em andamento
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("scrape_jobs")
  ...
  const criarConcorrente = async () => {
    ...
    if (!novoNome.trim() || !host) return toast.error("Informe nome e host");
  ...
  const iniciar = async () => {
    if (!selected) return toast.error("Selecione um concorrente");
    setStarting(true);
    ...
    setStarting(false);
    if (error || !data?.success) return toast.error(error?.message || data?.error || "Falha ao iniciar");
  ...
  return (
    <Card className="border-primary/30">
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Coleta de catálogo VTEX</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-[1fr_auto]">
          <Select value={selected} onValueChange={setSelected}>
            <SelectTrigger><SelectValue placeholder="Selecione o concorrente" /></SelectTrigger>
            <SelectContent>
              {concorrentes.map((c) => (
                <SelectItem key={c.id} value={c.id}>{c.nome} — {c.host} (sc {c.sales_channel})</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Button onClick={iniciar} disabled={starting || !!rodando} className="gap-2">
        ...
        <div className="grid gap-2 md:grid-cols-5 items-end rounded-lg border border-border/60 p-3">
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Label className="text-xs">Praça esperada</Label>
              <Input value={novaPraca} onChange={(e) => setNovaPraca(e.target.value)} placeholder="MONTEALTO" />
            </div>
            <Button variant="outline" size="icon" onClick={criarConcorrente} title="Cadastrar">
          ...
          <div className="space-y-3 rounded-lg border border-border/60 p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium flex items-center gap-2">
                ...
                {concJob?.nome || "Coleta"} — {job.status}
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs text-muted-foreground">
              ...
              <span>SKUs gravados: <strong className="text-foreground">{job.products_found || 0}</strong></span>
              <span>Com preço válido: <strong className="text-foreground">{job.skus_validos || 0}</strong></span>
              <span>Descartados por indisponibilidade: <strong className="text-foreground">{job.skus_indisponiveis || 0}</strong></span>
              <span>Sem EAN: <strong className="text-foreground">{job.skus_sem_ean || 0}</strong></span>
              <span>Rate limits (429): <strong className="text-foreground">{job.rate_limit_hits || 0}</strong></span>
            </div>
            ...
            <div className={`rounded-md border p-2 text-xs ${pracaDivergente ? "border-destructive/40 bg-destructive/10 text-destructive" : "border-border/60 text-muted-foreground"}`}>
              <span className="flex items-center gap-2">
                ...
                Lojista/praça retornado pela API:{" "}
                <strong className="text-foreground">{job.lojista_detectado || "—"}</strong>
                {concJob?.praca_esperada && <> (esperado: {concJob.praca_esperada})</>}
              </span>
              {pracaDivergente && <p className="mt-1">A praça retornada não é a esperada — os preços podem não corresponder à loja concorrente.</p>}
            </div>
            ...
            {!!job.categorias_incompletas?.length && (
              <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-2 text-xs text-amber-700">
                {job.categorias_incompletas.length} categoria(s) bateram no teto de 2.500 e foram refeitas por marca:
            ...
            <div className="max-h-48 overflow-auto rounded-md bg-muted/40 p-2 font-mono text-[11px] leading-relaxed">
              ...
              {!job.log_lines?.length && <div className="text-muted-foreground">Sem log ainda…</div>}
            </div>
          </div>
        )}
      </CardContent>
