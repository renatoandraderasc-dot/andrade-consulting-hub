...
AS $$
DECLARE
  v_cfg   public.store_vr_config%ROWTYPE;
  v_url   text;
...
BEGIN
  ...
  WITH linhas AS (
    SELECT l FROM jsonb_array_elements(v_json) AS l
  ), prep AS (
    SELECT
      ...
      l->>'ref'                                         AS ref
    FROM linhas
    WHERE COALESCE(l->>'origem','') <> 'TRANSFERENCIA'
  ), classif AS (
    SELECT p.*,
      COALESCE(
        ...
        (SELECT m.tipo FROM public.vr_lancamento_map m
          WHERE m.id_tipo = p.id_tipo AND m.store_id IS NULL LIMIT 1)
      ) AS tipo,
      COALESCE(
    ...
    SELECT p_store_id, v_user, c.data,
           ...
           COALESCE(c.tipo, 'Despesas'),
           COALESCE(c.subtipo, 'OUTROS'),
...
REVOKE EXECUTE ON FUNCTION public.importar_lancamentos_vr(uuid, date, date, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.importar_lancamentos_vr(uuid, date, date, uuid) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.importar_lancamentos_vr_auto() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.importar_lancamentos_vr_auto() TO authenticated, service_role;

SELECT cron.unschedule(jobid) FROM cron.job WHERE jobname = 'importar-lancamentos-vr-hora';
SELECT cron.schedule('importar-lancamentos-vr-hora', '7 * * * *',
  $cron$ SELECT public.importar_lancamentos_vr_auto(); $cron$);