CREATE OR REPLACE FUNCTION public.importar_lancamentos_vr(p_store_id uuid, p_inicio date, p_fim date, p_user_id uuid DEFAULT NULL)
...
BEGIN
  ...
  prep AS (
    SELECT (l->>'data_pagamento')::date AS data,
           ...
           NULLIF(l->>'ref','') AS ref_api,
           md5(concat_ws('|',
                 COALESCE(l->>'data_pagamento',''), COALESCE(l->>'documento',''),
                 COALESCE(l->>'fornecedor',''), COALESCE(l->>'valor_pago',''),
                 COALESCE(l->>'id_tipo',''))) AS chave
    FROM linhas WHERE COALESCE(l->>'origem','') <> 'TRANSFERENCIA'
  ), comref AS (
    SELECT p.*,
           COALESCE(p.ref_api,
                    p.chave || '-' || row_number() OVER (PARTITION BY p.chave)) AS ref
    FROM prep p
  ), dedup AS (
    SELECT DISTINCT ON (ref) * FROM comref WHERE ref IS NOT NULL
  ), classif AS (
    SELECT p.*,
    ...
    SELECT p_store_id, v_user, c.data,
           ...
           'ativo', 'VR', c.ref
    FROM classif c
    ON CONFLICT (store_id, origem, origem_ref) WHERE origem_ref IS NOT NULL
    DO UPDATE SET
...
CREATE TABLE IF NOT EXISTS public.titulo_planejamento (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  store_id uuid NOT NULL REFERENCES public.stores(id) ON DELETE CASCADE,
  titulo_ref text NOT NULL,
  fornecedor text,
  documento text,
  vencimento date,
  valor numeric,
  situacao text NOT NULL DEFAULT 'A negociar',
  data_prevista date,
  categoria text,
  responsavel text,
  ...
  UNIQUE (store_id, titulo_ref)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.titulo_planejamento TO authenticated;
GRANT ALL ON public.titulo_planejamento TO service_role;

ALTER TABLE public.titulo_planejamento ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins manage titulo_planejamento"
  ON public.titulo_planejamento FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'admin'))
  WITH CHECK (has_role(auth.uid(), 'admin'));

CREATE POLICY "Store users view titulo_planejamento"
  ON public.titulo_planejamento FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.user_store_access usa
                 WHERE usa.user_id = auth.uid() AND usa.store_id = titulo_planejamento.store_id AND usa.approved));

CREATE POLICY "Store users insert titulo_planejamento"
  ON public.titulo_planejamento FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.user_store_access usa
                 WHERE usa.user_id = auth.uid() AND usa.store_id = titulo_planejamento.store_id AND usa.approved));

CREATE POLICY "Store users update titulo_planejamento"
  ON public.titulo_planejamento FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.user_store_access usa
                 WHERE usa.user_id = auth.uid() AND usa.store_id = titulo_planejamento.store_id AND usa.approved))
  WITH CHECK (EXISTS (SELECT 1 FROM public.user_store_access usa
                 WHERE usa.user_id = auth.uid() AND usa.store_id = titulo_planejamento.store_id AND usa.approved));

CREATE TRIGGER trg_titulo_planejamento_updated
  BEFORE UPDATE ON public.titulo_planejamento
